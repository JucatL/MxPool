...
class SoftPoolingGcnEncoder(nn.Module):
    def __init__(self, max_num_nodes, num_aspect, multi_conv, multi_pool, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        # Processing features of convolution with full connection layer
        ...
        if self.multi_conv ==1:
            self.FC_first = Activation_Net(3,20,self.aspect) #这样是两层全连接层
            self.FC_after_conv = torch.nn.ModuleList()
            for i in range(num_pooling):
                self.FC_after_conv.append(Activation_Net(3,20, self.aspect)) #因为池化否学习到的embedding tensor的维度是不变的，所以这里的参数不用改变
        # Processing features of Pooling with full connection layer
        if self.multi_pool == 1:
            self.FC_pool = torch.nn.ModuleList()
            for i in range(num_pooling):
                self.FC_pool.append(Activation_Net(3, 10, self.aspect))
        ...
        self.conv_merge_fist = self.build_merge_conv_layers(
            hidden_dim[self.aspect - 1] * (num_layers - 1) + embedding_dim[self.aspect - 1],
            int(conv_merge_fist_dim / self.aspect), num_layers,
            add_self, normalize=True, dropout=dropout)
        if multi_conv == 1:
            self.pred_input_dim = []
        ...
        for i in range(num_pooling):
            embed_conv = self.build_merge_conv_layers(
                hidden_dim[self.aspect-1] * (num_layers - 1) + embedding_dim[self.aspect-1], int(sum(self.pred_input_dim) / self.aspect), num_layers,
                    add_self, normalize=True, dropout=dropout)
        ...
        for i in range(num_pooling):
            ...
            assign_pred = self.build_pred_layers(assign_pred_input_dim, [], assign_dim, num_aggs=1, aspect=self.aspect)#end_dim的位置是assign_dim
            assign_merge_layer = self.build_pred_layers(
                assign_hidden_dim[self.aspect-1] * (num_layers - 1) + assign_dim[self.aspect-1], [], [int((sum(assign_dim)) / self.aspect)])
    ...
    def forward(self, x, adj, batch_num_nodes=None, features=None, **kwargs):
        ...
        :param x:
        :param adj:
        :param batch_num_nodes: 用来判断用不用embedding-mask
        :param graphfeatures:
        :param kwargs:
        :return:
        '''
        ...
        if self.multi_conv == 1:
            ...
            embedding_tensor = torch.zeros_like(embedding_tensor_list[self.aspect-1])
            for s in range(self.aspect):
                leng = embedding_tensor_list[s].size()[2]
                for t in range(len(fc)):
                    embedding_tensor[t, :, :leng] = embedding_tensor[t, :, :leng] + embedding_tensor_list[s][t] * fc[t][s] #

            embed_ones = torch.eye(embedding_tensor.size()[1]).cuda()
            embedding_tensor = self.conv_merge_fist(embedding_tensor, embed_ones)
        ...
        for i in range(self.num_pooling):
            ...
            if self.multi_pool == 1:
                self.assign_tensor_list = self.gcn_forward(x_a, adj,
                                                  ...
                                                  embedding_mask, aspect=self.aspect)
                self.assign_tensor = torch.zeros_like(self.assign_tensor_list[self.aspect-1])
                if len(features) > 1:
                ...
                for s in range(self.aspect):
                    leng = self.assign_tensor_list[s].size()[2]
                    for t in range(len(fc_p)):
                        self.assign_tensor[t, :, :leng] = self.assign_tensor[t, :, :leng] + self.assign_tensor_list[s][t] * fc_p[t][s]
            ...
            if self.multi_conv == 1:
                ...
                for s in range(self.aspect):
                    leng = embedding_tensor_list[s].size()[2]
                    for t in range(len(fc)):
                        embedding_tensor[t, :, :leng] = embedding_tensor[t, :, :leng] + embedding_tensor_list[s][t] * \
                                                        fc[t][s]
                embed_ones = torch.eye(embedding_tensor.size()[1]).cuda()
                embedding_tensor = self.embed_merge_conv[i](embedding_tensor, embed_ones)
            else:
                embedding_tensor = self.gcn_forward(x, adj,
    ...
    def build_merge_conv_layers(self, input_dim, merge_dim, num_layers, add_self,
                                normalize=False, dropout=0.0):

        merge_conv = GraphConv(input_dim=input_dim, output_dim=merge_dim, add_self=add_self,
                               normalize_embedding=normalize, bias=self.bias)
    ...
    def construct_mask(self, max_nodes, batch_num_nodes):
        ...
        batch_num_nodes里面保存的是本batch中每个图有多少个节点
        '''
        # masks
